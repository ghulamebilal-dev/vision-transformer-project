        ...
        if uploaded_file is not None and 'results' in locals():
            ...
            if results['heatmap_overlay'] is not None:
                ...
                if attention_data is not None:
                    st.write(f"**Attention Statistics:**")
                    col1, col2, col3 = st.columns(3)
                    col1.metric("Max Attention", f"{attention_data.max():.3f}")
                    col2.metric("Mean Attention", f"{attention_data.mean():.3f}")
                    col3.metric("Attention Std", f"{attention_data.std():.3f}")
            else:
